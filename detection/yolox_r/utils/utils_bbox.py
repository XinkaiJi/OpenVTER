...
def decode_outputs(outputs, input_shape):
    ...
    #   获得每一个特征点属于每一个种类的概率
    #---------------------------------------------------#
    outputs[:, :, 12:] = torch.sigmoid(outputs[:, :, 12:])
    for h, w in hw:
        #---------------------------#
        #   根据特征层的高宽生成网格点
        #---------------------------#   
    ...
    outputs[..., 4:12] = outputs[..., 4:12] * strides
    #-----------------#
    #   归一化
    #-----------------#
    outputs[..., [0,2]] = outputs[..., [0,2]] / input_shape[1]
    outputs[..., [1,3]] = outputs[..., [1,3]] / input_shape[0]
    outputs[..., [4, 6,8,10]] = outputs[..., [4, 6,8,10]] / input_shape[1] # h x
    outputs[..., [5, 7,9,11]] = outputs[..., [5, 7,9,11]] / input_shape[0]
...
def non_max_suppression(prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        #   利用置信度进行第一轮筛选
        #----------------------------------------------------------#
        conf_mask = (image_pred[:, 13] * class_conf[:, 0] >= conf_thres).squeeze()
