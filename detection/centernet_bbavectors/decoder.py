...
class DecDecoder(object):
    ...
    def _topk(self, scores):
        ...
        # topk_clses = torch.div(topk_ind, self.K, rounding_mode='trunc').int()
    ...
    def ctdet_decode(self, pr_decs):
        ...
        if detections_shape[0]==1:
            # conf_mask = (detections[:, :, -2] > self.conf_thresh).float().unsqueeze(2)
            # detections = detections * conf_mask

            # detections_ind = torch.nonzero(detections[:,:, -2].squeeze()).squeeze()
            # detections = detections[:,detections_ind,:].view(detections_shape[0],-1,detections_shape[2])
            ...
            return detections
            # detections = detections.data.cpu().numpy()
            # detections = detections[detections[:,:,-2]!=0].reshape(detections_shape[0],-1,detections_shape[2])
            # return detections
        else:
            detections = detections.data.cpu().numpy()
            index_all = (scores > self.conf_thresh).cpu().numpy()
            out_det = []

            for i in range(detections_shape[0]):
                index = index_all[i,:,:].squeeze()
                detections_out = detections[i, index, :]
                out_det.append(detections_out)
            return out_det
