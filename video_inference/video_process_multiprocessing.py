...
def queue_img_process(origin_img_q,result_img_q,config_json,road_config,video_name,video_info,save_folder):
    '''
    视频处理进程
    :param origin_img_q:
    :param result_img_q:
    ...
    try:
        ...
        while 1:
            ...
            print('\rprocess origin queue size:%d,result queue size:%d, main frame:%d/%d, FPS:%.1f, remain time:%.2f min' % (
                origin_img_q.qsize(),result_img_q.qsize(), frame_index, all_num_frame, process_fps, remain_time / 60), end="", flush=True)
            # print(
            #     '\rprocess origin queue size:%d,result queue size:%d, main frame:%d/%d, FPS:%.1f, remain time:%.2f min' % (
            #         origin_img_q.qsize(), result_img_q.qsize(), frame_index, all_num_frame, process_fps,
            #         remain_time / 60))
            s_time = e_time
    finally:
...
def queue_img_output(result_img_q,config_json,video_name, main_video_width,save_folder):
    '''
    视频输出
    :param result_img_q: 保存结果的队列
    :param config_json: 视频config文件
    :param video_name: 视频名称
    :param main_video_width: 视频宽度
        ...
        while 1:
            ...
            if video_writer is None:
                ...
                out_path = os.path.join(save_folder,
                                        'tracking_output_' + video_name + '.mp4')
...
def _save_det_bbox(save_file_folder,video_name,det_bbox_result):
    ...
    file_path = os.path.join(save_file_folder,'det_bbox_result_'+video_name+'.pkl')
...
def run(config_json):
    try:
        ...
        origin_img_q = mp.Queue(maxsize=20)

        result_img_q = mp.Queue(maxsize=10)

        processes = [
            mp.Process(target=queue_img_put, args=(origin_img_q, config_json,save_folder)),
            mp.Process(target=queue_img_process, args=(origin_img_q, result_img_q, config_json, road_config, video_name,video_info,save_folder)),
            mp.Process(target=queue_img_output, args=(result_img_q,config_json,video_name, main_video_width,save_folder)),
        ]
        ...
        # 清理资源代码
        clear_resources(origin_img_q, result_img_q)

def clear_resources(origin_img_q, result_img_q):
    """
    清理资源，确保队列被正确关闭。
    """
    try:
        # 清理队列
        origin_img_q.close()
        origin_img_q.join_thread()
        result_img_q.close()
        result_img_q.join_thread()

    except Exception as e:
        print(f"Error while clearing resources: {e}")
